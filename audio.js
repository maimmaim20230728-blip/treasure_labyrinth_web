...
/* 桜：日本古謡「さくらさくら」正調・全曲版（PD・編曲せず旋律そのまま／1曲流してループ）
   都節（陰）音階＝ラA4(69) シ71 ド72 ミ76 ファ77・下のファ=65
   さくら さくら／のやまも さとも／みわたす かぎり／かすみか くもか／においぞ いずる／いざや いざや／みに ゆかん
   ※中間句は下のファ(65)へおりるのが正調（独特の物悲しさ） */
(() => {
  const N = [ // [拍, MIDI, 拍数]
    [0, 69, 1], [1, 69, 1], [2, 71, 2],                     // さ く ら〜
    [4, 69, 1], [5, 69, 1], [6, 71, 2],                     // さ く ら〜
    [8, 71, 1], [9, 69, 1], [10, 71, 1], [11, 72, 1],       // の や ま も（シ ラ シ ド）
    [12, 71, 1], [13, 69, 1], [14, 65, 2],                  // さ と も〜（シ ラ ファ↓）
    [16, 71, 1], [17, 69, 1], [18, 71, 1], [19, 72, 1],     // み わ た す
    [20, 71, 1], [21, 69, 1], [22, 65, 2],                  // か ぎ り〜（シ ラ ファ↓）
    [24, 76, 1], [25, 76, 1], [26, 77, 2],                  // か す み〜（ミ ミ ファ↑高）
    [28, 76, 1], [29, 72, 1], [30, 71, 2],                  // か く も か（ミ ド シ）
    [32, 72, 1], [33, 71, 1], [34, 72, 1], [35, 69, 1],     // に お い ぞ（ド シ ド ラ）
    [36, 71, 1], [37, 69, 1], [38, 65, 2],                  // い ず る〜（シ ラ ファ↓）
    [40, 69, 1], [41, 69, 1], [42, 71, 2],                  // い ざ や〜
    [44, 69, 1], [45, 69, 1], [46, 71, 2],                  // い ざ や〜
    [48, 71, 1], [49, 69, 1], [50, 71, 1], [51, 72, 1],     // み に ゆ か
    [52, 69, 4],                                            // ん〜〜〜（ラで終止）
  ];
  // 低いトニックの持続音のみ（編曲せず・琴の余韻ていど）
  const drone = [[0, 45, 8], [16, 45, 8], [32, 45, 8], [48, 45, 8], [64, 45, 8], [80, 45, 8], [96, 45, 8]];
  SONGS.mazeSakura = { bpm: 76, loopSteps: 112, tracks: [
    track('sine', 0.15, N.map(x => [x[0] * 2, x[1], x[2] * 2])),          // 旋律（そのまま）
    track('sine', 0.05, drone.map(x => [x[0] * 2, x[1], x[2] * 2])),      // 低音ドローン
  ]};
})();
/* 森：オルゴール調（maze3）を流用 */
