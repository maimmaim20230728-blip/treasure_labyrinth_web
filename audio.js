...
/* 桜：日本古謡「さくらさくら」正調全曲版（メロディはパブリックドメイン・Web Audioで自作合成）
   都節（陰）音階 ラ=A4(69) シ=71 ド=72 ミ=76 ファ=77・下のファ=65
   さくら さくら / のやまも さとも / みわたす かぎり /
   かすみか くもか（高音でひらく聴かせどころ）/ においぞ いずる / いざや いざや / みに ゆかん */
(() => {
  const N = [ // [拍, MIDI, 拍数]
    // さくら さくら（ラ ラ シ〜）
    [0, 69, 1], [1, 69, 1], [2, 71, 2],   [4, 69, 1], [5, 69, 1], [6, 71, 2],
    // のやまも さとも（ラ シ ド〜 / ド シ ラ〜）
    [8, 69, 1], [9, 71, 1], [10, 72, 2],  [12, 72, 1], [13, 71, 1], [14, 69, 2],
    // みわたす かぎり（くり返し）
    [16, 69, 1], [17, 71, 1], [18, 72, 2],  [20, 72, 1], [21, 71, 1], [22, 69, 2],
    // かすみか くもか（ミ ミ ファ〜 ミ / ド〜 シ）＝高音でひらく
    [24, 76, 1], [25, 76, 1], [26, 77, 2],  [28, 76, 1], [29, 72, 1], [30, 71, 2],
    // においぞ いずる（ド シ ド ラ / シ ラ ファ〜）＝おりてくる
    [32, 72, 1], [33, 71, 1], [34, 72, 1], [35, 69, 1],  [36, 71, 1], [37, 69, 1], [38, 65, 2],
    // いざや いざや（ラ ラ シ〜）
  ...
  // 琴らしい やわらかな分散和音（都節）
  const arp = [];
  for (let b = 0; b < 56; b += 4) {
    const base = [57, 64, 65, 69, 71]; // A3 E4 F4 A4 B4
    arp.push([b, base[(b / 4) % 5], 2], [b + 2, base[((b / 4) + 2) % 5] + 0, 2]);
  }
  SONGS.mazeSakura = { bpm: 74, loopSteps: 112, tracks: [
    track('sine', 0.14, N.map(x => [x[0] * 2, x[1], x[2] * 2])),           // 主旋律
    track('triangle', 0.05, arp.map(x => [x[0] * 2, x[1], x[2] * 2])),      // 琴の分散和音
    track('sine', 0.06, [[0, 45, 8], [16, 45, 8], [32, 45, 8], [48, 41, 8], [64, 45, 8], [80, 45, 8], [96, 45, 8]].map(x => [x[0] * 2, x[1], x[2] * 2])), // ドローン低音
  ]};
})();
/* 森：オルゴール調（maze3）を流用 */
