...
const S = {
  ...
  clearTimers: [], resumeAccum: 0, lastBest: null, // A-4クリア演出タイマー / B-2つづきから5秒計 / B-4じこベスト
};
const SAVE_KEY = 'tlab.v1';
const RESUME_KEY = 'tlab.resume'; // B-2 中断スナップショット（通常モード限定）
function loadSave() {
  let s = null;
  try { s = JSON.parse(localStorage.getItem(SAVE_KEY) || 'null'); } catch (e) {}
  const nd = L.DIFFS.length;
  const has = k => !!(s && typeof s === 'object' && k in s); // 旧セーブに無いフィールドかの判定（後方互換）
  S.save = Object.assign({ gender: 'm', exp: 0, badges: [], equipped: null, sound: true, fontScale: 0, volBgm: 1, volSfx: 1, replays: [], themeClears: [], zukanAllDone: false, lv99Celebrated: false, masterCelebrated: false, best: {}, vibe: false }, s || {});
  ...
  // B-1 迷宮ずかん：テーマ別クリア数（長さ9・後方互換パディング）
  if (!Array.isArray(S.save.themeClears)) S.save.themeClears = [];
  while (S.save.themeClears.length < THEMES.length) S.save.themeClears.push(0);
  // 一度きりの祝い演出フラグ。旧セーブ（フィールド無し）は既に到達済みなら演出済み扱いにして再発火を防ぐ
  if (!has('zukanAllDone')) S.save.zukanAllDone = S.save.themeClears.slice(0, THEMES.length).every(v => v > 0);
  if (!has('lv99Celebrated')) S.save.lv99Celebrated = L.levelFor(S.save.exp) >= 99;
  if (!has('masterCelebrated')) S.save.masterCelebrated = L.masteryScore(S.save.exp) >= L.SCORE_MAX;
  S.save.zukanAllDone = !!S.save.zukanAllDone;
  S.save.lv99Celebrated = !!S.save.lv99Celebrated;
  S.save.masterCelebrated = !!S.save.masterCelebrated;
  // B-4 じこベスト（{ "diffIdx"[+"s"]: 最小finMs } 形式・型ガード）
  if (typeof S.save.best !== 'object' || S.save.best === null || Array.isArray(S.save.best)) S.save.best = {};
  // B-5 振動フィードバック（既定OFF）
  S.save.vibe = !!S.save.vibe;
  if (!S.save.lang || !I18N.dict[S.save.lang]) S.save.lang = detectLang();
  sanitizeEquip();
  Snd.setEnabled(true); // 音のON/OFFは廃止。ミュートは設定の音量スライダー(0%)で行う
}
function persist() { try { localStorage.setItem(SAVE_KEY, JSON.stringify(S.save)); return true; } catch (e) { return false; } } // A-5 成功でtrue/失敗でfalse
/* ---- B-2 中断スナップショット（通常モード限定・つづきから） ---- */
...
function saveResume() {
  const st = S.stage;
  if (!st || S.screen !== 'play' || st.simple || st.rep || st.cleared) return; // 通常プレイ中のみ
  try {
    const c = st.char;
    const pos = c.moving ? { rx: c.moving.tx, ry: c.moving.ty } : { rx: c.rx, ry: c.ry }; // 確定マス
    const ladders = [];
    for (let i = 0; i < st.ladders.length; i++) if (st.ladders[i]) ladders.push([i, st.ladders[i]]);
    const chests = [];
    for (const pair of st.chests) if (!pair[1].opened) chests.push(pair[0]); // 未開封の宝箱のみ復元対象
    const snap = {
      v: 1, seed: st.seed, diffIdx: st.diffIdx, themeIdx: st.themeIdx,
      start: { rx: st.start.rx, ry: st.start.ry }, goal: { rx: st.goal.rx, ry: st.goal.ry },
      timerMs: Math.round(st.timer.ms), gameTime: Math.round(st.gameTime),
      charRx: pos.rx, charRy: pos.ry,
      counts: { pick: st.counts.pick, ladder: st.counts.ladder, coin: st.counts.coin, diamond: st.counts.diamond },
      pickCharges: st.pickCharges, ladderRetrievals: st.ladderRetrievals,
      tackleReadyAt: Math.round(st.tackleReadyAt), warpReadyAt: Math.round(st.warpReadyAt),
      hintReadyAt: Math.round(st.hintReadyAt || 0),
      ladders, chests, visits: Array.from(st.visits),
      brokenWalls: (st.brokenWalls || []).slice(), // 壊した壁index（seed再生成では無傷なので復元後に再適用する）
    };
    localStorage.setItem(RESUME_KEY, JSON.stringify(snap));
  } catch (e) {}
}
function saveResumeOnHide() { // visibilitychange(hidden)時：演出は完了状態(onEnd適用後)にしてから保存
  const st = S.stage;
  if (!st || S.screen !== 'play' || st.simple || st.rep) return;
  let guard = 5;
  while (st.cuts.active && guard-- > 0) st.cuts.skip(); // 連鎖する演出(のぼり→宝箱等)も完了させ、アイテム数まで確定してから保存
  if (st.cleared) { clearResume(); return; } // 演出完了でクリアに至ったら破棄
  saveResume();
}
function vibrate(pat) { if (S.save && S.save.vibe && navigator.vibrate) { try { navigator.vibrate(pat); } catch (e) {} } } // B-5
...
function startStage(di, simple) {
  ...
  const st = {
    ...
    hintReadyAt: 0, hintCells: null, hintUntil: 0,                     // B-3 ヒント（正解ルート先7マス）
    brokenWalls: [],                                                   // つるはし/体当たりで壊した壁index（つづきから復元用）
  };
    ...
    if (wx >= x && wx <= x + w && wy >= y && wy <= y + h) {
      ...
      if (mode === 'ladder') {
        ...
        st.cuts.start('break', via === 'pick' ? 2000 : 1500, { wi: cand.wi, via, d: cand.d, hitsDone: 0 }, () => {
          ...
          st.brokenWalls.push(cand.wi); // つづきから復元用に記録
...
function doClear() {
  ...
  if (!st.simple) clearResume(); // B-2 中断スナップショットは通常モードのクリア時だけ破棄（シンプルは触らない）
  Snd.sfx('goal');
  vibrate([60, 40, 60]); // B-5 ゴールの振動
  ...
  S.lastClearOk = ok; // 3) congratsを閉じた時のBGM復帰用（達成=clearBig / 未達=clearSoft）
  ...
  // B-1 迷宮ずかん：テーマ別クリア数を加算（シンプルモードも1件として数える）
  S.save.themeClears[st.themeIdx] = (S.save.themeClears[st.themeIdx] || 0) + 1;
  // 4) 祝い演出のフラグは「実際に表示できた時」に立てる。ここでは発火可否だけ判定（キャンセルされたら次のクリアで再挑戦）
  const zukanAllNow = !S.save.zukanAllDone && S.save.themeClears.slice(0, THEMES.length).every(v => v > 0);
  // B-4 じこベスト：難易度(＋シンプル)ごとの最短finを更新
  const bestKey = String(st.diffIdx) + (st.simple ? 's' : '');
  const prevBest = S.save.best[bestKey];
  const isNewBest = (prevBest == null || fin < prevBest);
  if (isNewBest) S.save.best[bestKey] = Math.round(fin);
  S.lastBest = { val: S.save.best[bestKey], isNew: isNewBest };
  ...
  // 4) 到達済み判定はprev比較をやめ「未祝い＆現在到達」に。演出が実表示された時にフラグ確定
  const gotMaster = newScore >= L.SCORE_MAX && !S.save.masterCelebrated;
  ...
  clearClearTimers(); // A-4 前回のクリア演出タイマーが残っていれば破棄
  S.clearTimers.push(setTimeout(() => {
    if (!S.stage) return; // ✕やめる等でステージが消えていたら中断（クラッシュ回避）
    ...
    // B-1 9テーマ全クリアの初回はトースト＋紙吹雪で祝う（実表示できた時にフラグ確定＝早押しキャンセルなら次回再挑戦）
    if (zukanAllNow) S.clearTimers.push(setTimeout(() => {
      if (!S.stage) return;
      toast(t('zukanAll')); spawnConfetti(90);
      S.save.zukanAllDone = true; persist();
    }, 1500));
    // 🎉最高難易度クリア or LV99到達 or 🏆トレジャーマスター達成 → 大演出
    const gotLv99 = newLv >= 99 && !S.save.lv99Celebrated;
    if (isUlt || gotLv99 || gotMaster) S.clearTimers.push(setTimeout(() => {
      if (!S.stage) return;
      showCongrats(isUlt, gotLv99, gotMaster);
      if (gotLv99) S.save.lv99Celebrated = true;   // 実表示できた時にだけ確定（キャンセルされたら次のクリアで再発火）
      if (gotMaster) S.save.masterCelebrated = true;
      if (gotLv99 || gotMaster) persist();
    }, 1100));
  }, 650));
}
function clearClearTimers() { S.clearTimers.forEach(clearTimeout); S.clearTimers = []; } // A-4
...
function buildClearOverlay(raw, fin, ok, e, lv, coinF, diaF, rew) {
  const st = S.stage; if (!st) return; // A-4 ステージが無ければ描画しない（保険）
  const c = st.counts;
  ...
  // B-4 じこベスト（自己基準のみ・順位や他人比較は出さない）
  if (S.lastBest) {
    html += '<div class="crow">' + t('best') + ' <b>' + fmt(S.lastBest.val) + '</b>'
      + (S.lastBest.isNew ? ' <span class="bestNew">' + t('bestNew') + '</span>' : '') + '</div>';
  }
  ...
  if (ok) {
    ...
    const tag = rew.fixed ? '🏆 ' + t('special') : 'EXP ×' + rew.mult;
...
function updateReplay(st, dt) {
  ...
  if (r.playing) { r.t += dt * (r.spd || 1); if (r.t >= r.dur) { r.t = r.dur; r.playing = false; updateReplayBar(st); } } // B-6 倍速
  if (n >= 4) {
...
function buildReplayStage(rep) {
  ...
  const st = {
    ...
    rec: null, rep: { fr, t: 0, dur, playing: true, spd: 1 }, // B-6 倍速（×1→×2→×4）
    ...
    hintReadyAt: 0, hintCells: null, hintUntil: 0, brokenWalls: [],
  };
  st.theme = THEMES[st.themeIdx];
  st.cuts = new L.CutsceneCtrl(st.timer);
  st.trailCol = makeTrail(st.theme);
...
/* ---- B-2 中断スナップショットからの復元（通常モード限定） ----
   seedから迷路＋部屋を同一に再生成し、保存済みの進行状態（位置/タイム/アイテム/CT/ハシゴ/宝箱/足あと）を書き戻す。
   復元に失敗（v不一致・範囲外・パース失敗）した場合はnullを返してサイレント破棄する。 */
function buildResumeStage(snap) {
  if (!snap || snap.v !== 1) return null;
  const di = snap.diffIdx;
  if (typeof di !== 'number' || di < 0 || di >= L.DIFFS.length) return null;
  const diff = L.DIFFS[di];
  if (!snap.start || !snap.goal || !snap.counts) return null;
  const seed = snap.seed >>> 0;
  const m = L.generate(diff.w, diff.h, seed, { rooms: true }); // 通常モードは部屋あり（同オプション）
  const start = { rx: snap.start.rx | 0, ry: snap.start.ry | 0 };
  const goal = { rx: snap.goal.rx | 0, ry: snap.goal.ry | 0 };
  if (start.rx < 0 || start.rx >= diff.w || start.ry < 0 || start.ry >= diff.h) return null;
  if (goal.rx < 0 || goal.rx >= diff.w || goal.ry < 0 || goal.ry >= diff.h) return null;
  const themeIdx = clamp(snap.themeIdx | 0, 0, THEMES.length - 1);
  const path = L.shortestPath(m, start.rx, start.ry, goal.rx, goal.ry, null);
  if (!path) return null;
  const lv = L.levelFor(S.save.exp);
  sanitizeEquip();
  const eq = new Set(S.save.equipped);
  const cRx = clamp(snap.charRx | 0, 0, diff.w - 1), cRy = clamp(snap.charRy | 0, 0, diff.h - 1);
  const cnt = snap.counts;
  const st = {
    diffIdx: di, diff, m, start, goal, lv, eq, simple: false, seed,
    rec: null, rep: null,
    ladders: new Uint8Array(m.bw * m.bh),
    chests: new Map(),
    timer: new L.StageTimer(),
    plan: [], gameTime: Math.max(0, snap.gameTime | 0),
    targetMs: L.targetSeconds(diff, path.length - 1) * 1000,
    counts: {
      pick: clamp(cnt.pick | 0, 0, L.ITEM_CAPS.pick),
      ladder: Math.max(0, cnt.ladder | 0),
      coin: clamp(cnt.coin | 0, 0, L.ITEM_CAPS.coin),
      diamond: clamp(cnt.diamond | 0, 0, L.ITEM_CAPS.diamond),
    },
    char: { rx: cRx, ry: cRy, x: roomX(cRx), y: roomY(cRy), dir: 2, frame: 0, animT: 0, moving: null },
    cam: { x: roomX(cRx), y: roomY(cRy) },
    themeIdx,
    particles: [], shake: 0, cleared: false,
    tackleReadyAt: Math.max(0, snap.tackleReadyAt | 0),
    speed: (eq.has('speed') ? L.speedCells(lv) : L.CHAR_SPEED) + (eq.has('hansel') ? L.hanselWalkBonus(lv) : 0),
    viewRange: eq.has('hawk') ? L.viewRangeFor(lv) : 7,
    pickCharges: Math.max(0, snap.pickCharges | 0),
    ladderRetrievals: Math.max(0, snap.ladderRetrievals | 0),
    warpReadyAt: Math.max(0, snap.warpReadyAt | 0), warpOk: false,
    hanselShades: eq.has('hansel') ? L.hanselShades(lv) : 0,
    visits: new Uint8Array(diff.w * diff.h),
    clones: [], cloneSpeed: L.cloneSpeed(lv),
    hintReadyAt: Math.max(0, snap.hintReadyAt | 0), hintCells: null, hintUntil: 0,
    brokenWalls: Array.isArray(snap.brokenWalls) ? snap.brokenWalls.slice() : [],
  };
  st.theme = THEMES[themeIdx];
  st.cuts = new L.CutsceneCtrl(st.timer);
  // ハシゴ復元（非0のみ・方向値1〜4）
  if (Array.isArray(snap.ladders)) for (const pr of snap.ladders) {
    const idx = pr[0] | 0, val = pr[1] | 0;
    if (idx >= 0 && idx < st.ladders.length && val >= 1 && val <= 4) st.ladders[idx] = val;
  }
  // 未開封の宝箱を復元（開封済みはcountsに反映済みなので置かない）
  if (Array.isArray(snap.chests)) for (const idx of snap.chests) {
    const ri = idx | 0;
    if (ri >= 0 && ri < diff.w * diff.h) st.chests.set(ri, { opened: false, result: null });
  }
  // 足あと復元
  if (Array.isArray(snap.visits)) for (let i = 0; i < st.visits.length && i < snap.visits.length; i++) st.visits[i] = Math.min(3, snap.visits[i] | 0);
  st.visits[cRy * diff.w + cRx] = Math.max(1, st.visits[cRy * diff.w + cRx]);
  st.trailCol = makeTrail(st.theme);
  st.roomFloor = mixHex(st.theme.floor, st.theme.top, 0.33);
  st.roomFloor2 = mixHex(st.theme.floor2, st.theme.top2, 0.33);
  st.roomMask = new Uint8Array(m.bw * m.bh);
  ...
  { let ct = Infinity, pct = 0;
    ...
    if (eq.has('hansel') && L.hanselWarp(lv)) { ct = Math.min(ct, L.HANSEL_WARP_CT); pct += L.HANSEL_WARP_PCT; }
    st.warpPct = Math.min(100, pct); st.warpCT = ct; }
  st.goalArrow = (eq.has('hawk') && lv >= 15) || (eq.has('hansel') && L.hanselGoalArrow(lv));
  // 1) 壊した壁を再適用（seed再生成では無傷なので、壁が復活してつるはし消費だけ残る非対称を防ぐ）。
  //    targetMs/goalDistは無傷の迷路基準で算出済み＝原本のステージと一致。壁破壊は通路を増やすだけで詰みは生じない。
  for (const idx of st.brokenWalls) {
    const wi = idx | 0;
    if (wi >= 0 && wi < m.g.length && m.g[wi] === 1) m.g[wi] = 0; // 壊せる壁(1)のみ床(0)へ
  }
  st.timer.ms = Math.max(0, snap.timerMs | 0);
  spawnClones(st);
  return st;
}
function startResume(snap) {
  let st;
  try { st = buildResumeStage(snap); } catch (e) { st = null; }
  if (!st) return false;
  S.stage = st; S.zoom = 1; S.targetMode = null; S.tracing = false; S.resumeAccum = 0;
  showScreen('play');
  updateSlots(); descDefault();
  $('diffName').textContent = t('diffs')[st.diffIdx];
  $('timeTarget').textContent = t('target') + ' ' + fmt(st.targetMs);
  showStageName(t('themes')[st.themeIdx]);
  Snd.bgm(THEME_BGM[st.themeIdx]);
  return true;
}
function updateReplayBar(st) {
  ...
  $('btnRepSpeed').textContent = '×' + (r.spd || 1); // B-6 倍速表示
  $('repTime').textContent = fmt(r.t) + ' / ' + fmt(r.dur);
}
function replaySpeedCycle() { // B-6 ×1→×2→×4を循環（再生終了↻でも維持）
  const st = S.stage; if (!st || !st.rep) return;
  const seq = [1, 2, 4];
  st.rep.spd = seq[(seq.indexOf(st.rep.spd || 1) + 1) % seq.length];
...
function saveCurrentReplay() {
  ...
  const backup = S.save.replays.slice();        // A-5 保存失敗時に元へ戻すためのバックアップ
  S.save.replays.unshift(S.pendingReplay);      // 新しいものを先頭へ
  while (S.save.replays.length > 9) S.save.replays.pop(); // 最大9件（古いものを消す）
  if (!persist()) {                             // 保存に失敗＝偽の成功表示をしない
    S.save.replays = backup;
    $('btnSaveReplay').textContent = t('saveFail'); // disabledにしない＝もう一度押せる
    Snd.sfx('nope');
    return;
  }
...
/* A-1 ワープ座標の確定：tick（前半700ms経過）とonEnd（早期スキップ含む）の両方から呼ばれ、
   movedフラグで二重適用を防ぐ。climb/breakと同じ「onEndで状態確定」構造に統一。 */
function applyWarpDest(st, data) {
  if (data.moved) return;
  data.moved = true;
  const c = st.char;
  c.rx = data.tx; c.ry = data.ty;
...
function doWarp() { // ⑧1回だけランダムワープ。ゴールから10マス以内には飛ばない
  ...
  setTarget(null);
  Snd.sfx('warp');
  st.cuts.start('warp', 1400, { fx: st.char.x, fy: st.char.y, tx: dest % st.m.w, ty: (dest / st.m.w) | 0, moved: false }, cut => {
    applyWarpDest(st, cut.data); // 前半スキップでも必ずここで座標確定（CTだけ消費して動かない不具合の修正）
    toast(t('warped'));
    onEnterRoom();
  });
  updateSlots();
}
/* B-3 ヒント💡：正解ルートの先7マスを約2.5秒 金色ドットで光らせる（CT60秒・EXPペナルティ無し・シンプルでも可） */
function useHint() {
  if (!playActive()) return; // リプレイ再生中/演出中/クリア後は無効
  const st = S.stage;
  const remain = (st.hintReadyAt || 0) - st.gameTime;
  ...
  if (!p || p.length < 2) { Snd.sfx('nope'); return; }
  st.hintCells = p.slice(1, 8);            // 現在地の次から先7マス
  st.hintUntil = st.gameTime + 2500;       // 約2.5秒フェード
  st.hintReadyAt = st.gameTime + 60000;    // CT60秒（gameTime基準）
  Snd.sfx('tap');
  updateHud();
}
...
function hideCongrats() {
  ...
  // 3) クリア画面に戻る時だけBGM復帰。達成=clearBig / 未達=clearSoft（未達クリアで化ける不具合の修正）
  if (S.screen === 'clear') Snd.bgm(S.lastClearOk ? 'clearBig' : 'clearSoft');
}
  ...
  if (st && S.screen === 'play' && st.rep) {
    ...
    if (cutA) {
      if (cutA.kind === 'break') { // つるはし=3回叩く / 体当たり=2回突進
        ...
        if (cutA.t >= cutA.dur / 2) applyWarpDest(st, cutA.data); // 確定はonEndと共通（movedで二重防止）
      }
    } else {
      ...
      // B-2 中断スナップショット：通常モードのプレイ中だけ5秒おきに保存（演出中はこのbranchに来ない）
      if (!st.simple && !st.rep) {
        S.resumeAccum += dt;
        if (S.resumeAccum >= 5000) { S.resumeAccum = 0; saveResume(); }
      }
    }
  }
  if (st) {
...
function updateHud() {
  ...
  // B-3 ヒント💡ボタンのCT表示（cntWarpと同方式・残り秒 or 💡）
  const remainH = Math.max(0, (st.hintReadyAt || 0) - st.gameTime);
  $('btnHint').textContent = remainH > 0 ? Math.ceil(remainH / 1000) + '' : '💡';
  $('btnHint').classList.toggle('dim', remainH > 0);
}
...
function render() {
  ...
  if (st) {
    ...
    drawPlan(st);
    drawHint(st); // B-3 ヒントの金色ドット（正解ルート先7マス）
...
function drawHint(st) { // B-3 正解ルートの先頭マスを金色ドットで（drawPlanと同型・最後にフェード）
  if (!st.hintCells || !st.hintCells.length) return;
  const remain = st.hintUntil - st.gameTime;
  if (remain <= 0) { st.hintCells = null; return; }
  const a = Math.min(1, remain / 800); // 最後の0.8秒でフェードアウト
  ctx.fillStyle = 'rgba(245,197,66,' + (0.85 * a).toFixed(3) + ')';
  for (const cell of st.hintCells) {
    ctx.beginPath(); ctx.arc(roomX(cell[0]), roomY(cell[1]), 4.2, 0, 6.3); ctx.fill();
  }
}
...
function showScreen(name) {
  // A-7 Web版SW更新の反映：安全な画面（タイトル/難易度選択）に来た時だけ1回リロード（プレイ中は発火しない）
  if ((name === 'title' || name === 'diff') && window.__swReloadPending && !window.__swReloading) {
    window.__swReloading = true; location.reload(); return;
  }
  S.screen = name;
  ...
  if (name === 'title') { $('title').classList.remove('hidden'); refreshTitle(); Snd.bgm('title'); }
    ...
    $('btnHint').classList.toggle('hidden', isRep); // B-3 リプレイ中はヒント非表示
  }
...
function buildSettings() {
  ...
  // B-5 振動フィードバック（既定OFF・端末が非対応なら効果なし）
  $('vibeLabel').textContent = t('vibe');
  const vt = $('vibeToggle');
  vt.textContent = S.save.vibe ? 'ON' : 'OFF';
  vt.classList.toggle('sel', S.save.vibe);
  vt.onclick = () => {
    S.save.vibe = !S.save.vibe; persist(); Snd.sfx('tap');
...
function applyLang() {
  ...
  $('btnBackTitle4').textContent = t('back'); // A-2 設定画面の「← タイトルへ」翻訳漏れ修正
  $('btnBackTitle5').textContent = t('back'); // A-2 リプレイ画面の「← タイトルへ」翻訳漏れ修正
  const hintBtn = $('btnHint'); hintBtn.title = t('hint'); hintBtn.setAttribute('aria-label', t('hint')); // B-3
...
function refreshTitle() {
  ...
  // B-2 つづきから：中断スナップショットがある時だけ表示（▶はコード側で付与）
  const snap = loadResume();
  $('btnResume').classList.toggle('hidden', !snap);
...
function buildDiffList() {
  ...
  L.DIFFS.forEach((d, i) => {
    ...
    const bt = S.save.best[String(i)]; // B-4 通常モードのじこベストがあれば併記
    const small = d.w + '×' + d.h + (bt != null ? '・⏱' + fmt(bt) : '');
    b.innerHTML = '<span>' + (locked ? '🔒 ' : '') + t('diffs')[i] + '</span><small>' + small + '</small>' + badge;
    b.onclick = () => {
  ...
  buildZukan(); // B-1 難易度選択の下に迷宮ずかん
}
/* B-1 迷宮ずかん：9テーマのクリアコレクション（各テーマをクリア済みなら✓・未クリアはグレー） */
function buildZukan() {
  const box = $('zukanBox'); if (!box) return;
  const clears = S.save.themeClears || [];
  const themes = t('themes');
  let html = '<div class="zukanHead">' + t('zukan') + '</div><div class="zukanGrid">';
  for (let i = 0; i < THEMES.length; i++) {
    const done = (clears[i] || 0) > 0;
    html += '<div class="zukanCell' + (done ? ' done' : '') + '">' + (done ? '✓ ' : '') + themes[i] + '</div>';
  }
  html += '</div>';
  box.innerHTML = html;
}
...
function attachUI() {
  ...
  // B-2 つづきから：中断スナップショットを復元。失敗時はサイレント破棄してボタンを消す
  $('btnResume').onclick = () => {
    Snd.sfx('tap');
    const snap = loadResume();
    if (!snap || !startResume(snap)) { clearResume(); refreshTitle(); }
  };
  ...
  $('btnRepSpeed').onclick = replaySpeedCycle; // B-6 倍速切替
  $('congrats').onclick = hideCongrats;
  // B-2 中断スナップショット：バックグラウンド化した瞬間に保存
  document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'hidden') saveResumeOnHide(); });
  $('volBgmR').oninput = () => {
  ...
  $('btnQuit').onclick = () => {
    ...
    if (Date.now() - quitArm < 2000) {
      clearClearTimers(); // A-4 演出タイマー破棄
      if (S.stage && !S.stage.simple) clearResume(); // B-2 中断スナップショットは通常モードのやめる時だけ破棄
      S.stage = null; quitArm = 0; showScreen('diff');
    } else { quitArm = Date.now(); toast(t('quit')); }
  };
  $('btnRetry').onclick = () => { Snd.sfx('tap'); clearClearTimers(); const p = S.pendingLevelUp; startStage(S.stage.diffIdx, S.stage.simple); S.pendingLevelUp = p; playPendingLevelUp(); };
  $('btnNext').onclick = () => { Snd.sfx('tap'); clearClearTimers(); S.stage = null; showScreen('diff'); playPendingLevelUp(); };
  // アイテムスロット
  $('slotPick').onclick = () => {
...
window.DM = {
  ...
  // 追加機能の機械検証用フック
  useHint, doClear, saveResume, saveResumeOnHide, startResume, buildResumeStage, loadResume, clearResume,
  showScreen, refreshTitle, buildDiffList, buildSettings, updateHud, replaySpeedCycle, startReplay,
  loadSave, clearClearTimers, hideCongrats, Snd,
};
})();
